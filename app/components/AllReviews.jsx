...
export class AllReviews extends React.Component {
  ...
  //sets graphs according to screen size
  handleResize = () => {
    this.setState({
  ...
  componentDidMount = () => {
    ...
    this.handleResize();
    //goes through listings data and increments values
  ...
  //handles graph clicks
  clickHandler(d, e) {
    this.setState({showToolTip: true, y: d.y, x: d.x})
  }
  //creates description at bottom of graph once clicked
  createTooltip() {
    if (this.state.showToolTip) {
      return (
        <div>
          <a data-tip data-for='click'>
            The average rating is a {this.state.y.toFixed(2)} with a {this.state.x} cancellation policy</a>
        </div>
      )
    }
  }
  render() {
    return (
      <div className="grid-x align-center">
        <div className="cell">
          <div ref="component">
            <LineChart width={this.state.componentWidth} height={this.state.componentWidth / 2} lineColors={['red']} dataPoints axisLabels={{
              x: 'All Cancellation Policy',
              y: 'All Overall Rating'
            }} grid axes xType={'text'} interpolate={'cardinal'} data={[
              [
                {
                  x: 'Flexible',
                  y: this.state.cancelFlexibleScore / this.state.cancelFlexibleCount
                }, {
                  x: 'Moderate',
                  y: this.state.cancelModerateScore / this.state.cancelModerateCount
                }, {
                  x: 'Strict',
                  y: this.state.cancelStrictScore / this.state.cancelStrictCount
                }
              ]
